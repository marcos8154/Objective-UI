import { CustomListItemTemplate } from "./CustomListItemTemplate";
import { DataGridColumnDefinition, ListItem, Mask, RadioOption, UIButton, UICheckBox, UIDataGrid, UIHead, UIList, UIRadioGroup, UISelect, UISpinner, UITextBox, UIView, ViewLayout, Widget } from "./Objective-UI";

export class DemoWidgets extends UIView
{
    //#region widgets declarations
    private buttons = [
    ...
    private radioGroup = new UIRadioGroup({
    ...
    private select = new UISelect({ name: 'select', title: 'UISelect:' });
    private list = new UIList({ name: 'listView' })
    private dataGrid = new UIDataGrid({ name: 'dtgCustomers' });
    //#endregion
    ...
    public composeView(): void
    {
        this.addWidgets('buttons', ...this.buttons);
        this.addWidgets('textBox', ...this.textBoxes);
        this.addWidgets('checkbox', ...this.checkBoxes);
        this.addWidgets('uiheads', ...this.heads);
        this.addWidgets('uiSpinner', ...this.spinners);
